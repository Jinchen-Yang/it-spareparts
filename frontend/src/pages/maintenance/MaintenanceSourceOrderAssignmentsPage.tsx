...
function canWriteAssignments(): boolean {
  try {
    ...
    if (role !== "admin" && role !== "boss") return false;
    const permissions = JSON.parse(localStorage.getItem("permissions") || "{}");
    return role === "admin" || (permissions?.page_maintenance === true
    ...
    try {
      ...
      if (actionRow) {
        ...
        if (next) {
          ...
          setActionRow(null);
          setWriteError("该来源维保单已删除、失效或不再属于你的可见范围，请关闭后重新选择");
          return;
        }
      } else {
        const freshById = new Map(fresh.map((row) => [row.raw_order_id, row]));
        setSelectedRows((previous) => previous
          .map((row) => freshById.get(row.raw_order_id))
          .filter((row): row is MaintenanceSourceOrderRow => row !== undefined));
        if (fresh.length !== draftIds.size) {
          setWriteError("部分来源维保单已删除、失效或不再属于你的可见范围，已从本批次移除");
          return;
        }
      }
