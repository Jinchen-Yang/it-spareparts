...
vi.mock("../../../api/maintenanceProjects", () => ({
  ...
  searchMaintenanceProjects: (...args: unknown[]) => searchMaintenanceProjects(...args),
}));
...
describe("MaintenanceSourceOrderAssignmentsPage", () => {
  ...
  it("冲突刷新未返回所选单号时移除失效条目并保留原因", async () => {
    localStorage.setItem("role", "admin");
    localStorage.setItem("permissions", JSON.stringify({
      page_maintenance: true,
      data_profit: true,
    ...
    await waitFor(() => expect(listMaintenanceSourceOrders).toHaveBeenCalledTimes(2));
    expect(within(dialog).getByText(/已明确选择 0 张来源维保单/)).toBeInTheDocument();
    expect(within(dialog).getByText(/部分来源维保单已删除、失效或不再属于你的可见范围/))
      .toBeInTheDocument();
    expect(within(dialog).getByDisplayValue("保留无法刷新的草稿")).toBeInTheDocument();
    expect(within(dialog).getByText("XM-001 · 稳定项目甲")).toBeInTheDocument();
  });

  it("项目输入搜索使用 POST 安全搜索而不是把关键字放进 URL", async () => {
    localStorage.setItem("role", "admin");
    searchMaintenanceProjects.mockResolvedValue({
      data: { rows: [ACTIVE_PROJECT], total: 1, page: 1, page_size: 100 },
    });
    ...
    fireEvent.change(screen.getByRole("combobox", { name: "稳定项目筛选" }), {
      target: { value: "稳定项目甲" },
    });

    await waitFor(() => expect(searchMaintenanceProjects).toHaveBeenCalledWith({
      q: "稳定项目甲",
      page: 1,
      page_size: 100,
      include_inactive: true,
    }));
    expect(listMaintenanceProjects).not.toHaveBeenCalledWith(
