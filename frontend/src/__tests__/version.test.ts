...
describe("v1.34.0 release notes (archived)", () => {
  it("publishes the spend breakdown tab with granularity and salesperson views", () => {
    const v134 = CHANGELOG.find((entry) => entry.version === "1.34.0")!;
    expect(v134.date).toBe("2026-09-17");
    const notes = v134.items.join("\n");
