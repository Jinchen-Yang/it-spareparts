...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    date: "2026-07-28",
    items: [
      "维保毛利统一按 XSDD 销售合同归集，正式名称为“合同级备件毛利”和“合同级贡献毛利”；同时保留含税、未税两套收入、成本、毛利及毛利率，管理员可设置默认显示含税、未税或两者同时显示，后端不会因默认项而丢弃另一套数据",
      "成本缺失行新增安全补价瀑布：互通 PN 池采购数量加权均价 → 池销售均价 → 本 PN 历史采购 → 本 PN 历史销售；只使用出库日当日或以前的有效样本，并保留参考方向、池版本、样本窗口、样本数、追溯月份和低置信提示",
      "合同收入、双税成本或税率证据不完整时，对应合同级备件毛利保持空值并说明原因，不用 0 或另一税口径代替；项目追踪工作簿报销明细尚未建立费用全量数据水位，合同级贡献毛利继续保持空值并标记“费用数据未就绪”",
      "项目页、API、Agent、CSV、订单 Excel、单合同工作簿和时间范围内一键批量工作簿统一双税口径与权限脱敏；无利润权限不返回毛利数值、状态、筛选或排序结论，无成本或利润权限不能导出合同工作簿",
      "本版本日志为待发布说明，代码完成不等于生产批准；税率 0%/缺失、费用税务字段、管理员默认或强制展示、期间口径、重复 XSDD、成本参考池资格，以及报销明细快照的合同覆盖范围、截止日、完整性确认和后续增量/替换规则须经甲方确认并完成发布门禁后才可上线",
    ],
  },
  {
    version: "1.18.0",
