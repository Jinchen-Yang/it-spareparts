...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    date: "2026-09-23",
    items: [
      "领用与返还页面改为现场操作优先：登记返还置于首屏，返还台账排在领用流程之前",
      "返还区按登记动作、概况、搜索台账、需求单核对排列；Excel 批量维护下移到领用明细区",
      "新增返还、领用和批量维护分区标题，改善长页面定位和辅助技术阅读顺序",
    ],
  },
  {
    version: "1.36.2",
  ...
  },
  {
    ...
    date: "2026-09-20",
    items: [
      "返还的来源单号/凭据上限扩大到 16384 字符；逐件 SN 支持多行扫码、数量核对和重复检查，长内容不会静默截断",
      "需求支持页面单条及批量新增、修改，并可选择字段撤销；已有人工修改遇到导入冲突时会提示核对，需求单仍可作废和恢复",
      "回款可按合同编号单条或批量登记、修改、作废；恢复先回到待确认，避免直接计入已确认累计",
      "人工领用可一次录入多条明细，预览成本后登记、更正或作废；保留返还义务和操作历史，不改变库存",
      "返还支持页面批量登记、修改和作废；批量结果逐条显示，已成功记录不重复提交，结果未知时保留原请求供核对",
    ],
  },
  {
    version: "1.35.0",
  ...
  },
  {
    ...
    date: "2026-09-16",
    items: [
      "维保数据分析新增全字段筛选：项目、客户、销售、需求单号、需求类型、仓库、成本来源，与时间范围、业务类型和 PN 搜索叠加；筛选写入网址，刷新保留，重置一键清空",
      "项目筛选支持按名称/编号搜索后多选；成本来源按系统关联、估算、人工回填、缺失四类筛选；仓库候选来自实际出库数据",
      "业务类型新增「拆改配服务」：维保主页筛选、项目面板「编辑基本信息」与数据分析筛选同步支持六档",
    ],
  },
  {
    version: "1.32.0",
    date: "2026-09-16",
    items: [
      "项目「备件与需求单」及「需求单与数据同步」补齐每张需求单的收货地址、联系人、电话；支持复制，按客户信息权限展示",
      "维保数据分析新增业务类型多选筛选，按需求单当前所属项目的整体维保、备件维保、算力运维、非维保或未标注分类，与时间范围和 PN 搜索叠加",
      "分类筛选同步作用于备件总成本、数量、图表、PN 排名、成本占比及坏件返还的项目范围；默认全部，清空恢复全部，刷新保留筛选",
    ],
  },
  {
    version: "1.31.0",
  ...
  },
  {
    ...
    date: "2026-09-13",
    items: [
      "领用与返还页新增实际收货台账：按项目登记，可关联需求单，支持修改、转移、作废和操作历史；返件 PN 无需匹配领用 PN",
      "项目卡显示已返还数量与有效需求备件数量的比例，全部件况计入，数据不足时不显示百分比，超过 100% 如实展示",
      "领用列表补充备件描述、SN、现场备注与返还要求；Excel 的是、否和清空与页面同步，改免返保留已收到的返件",
      "领用表下载附带实际收货只读快照，按项目和需求单汇总；统计列被改动时明确提示",
      "入库单支持后台预览与重复导入检查，拆旧返件和旧库退返均可入账；整机计 1 台，小数保留待审，来源更正须确认并留痕",
      "登记失败可重试而不重复计数；同时编辑会提示版本冲突，权限拒绝与校验失败也保留审计",
    ],
  },
  {
    version: "1.29.0",
    date: "2026-09-08",
    items: [
      "维保主页新增「业务类型」筛选（整体维保 / 备件维保 / 算力运维 / 非维保 / 未标注），与进行中、已结束等期限页签**叠加**使用而不是二选一",
      "默认五档全选、不排除任何项目：目前绝大多数项目还没标业务类型，默认排除会让卡墙整面变空；主动取消勾选才开始收窄",
      "被筛掉的项目数常驻显示「已按业务类型隐藏 N 个项目」并可一键「查看全部」，项目不会被悄悄挡掉；项目卡上直接标出业务类型，没标的显示「未标注业务类型」",
      "项目清单导出与批量移交下载跟随同一个业务类型筛选，所见即所得，不会出现屏幕上三个项目、下载下来是全量",
      "销售订单导入建项时把源表业务类型一并落库（此前解析出来就丢了），项目面板「编辑基本信息」也可人工补录或清空业务类型，改动留审计",
      "项目总表回传修复：只改一个备注上传，不再把同事在你下载之后改过的报销金额、累计实收、领用数量悄悄改回旧值（04 费用报销 / 05 实收回款 / 06 领用返还三张表）",
      "报销导入修复：同一笔报销明细出现在两张报销页且金额不同时，不再按 Sheet 先后顺序静默取第一条，改为明确报冲突、两条都不入账",
      "收款单导入支持一张收款单合并多个维保订单的导出形态（合并单元格按块读取），此前只认每张单的第一行；非维保订单与其他收款不再报红，按「不是维保、跳过」处理",
    ],
  },
  {
    version: "1.28.1",
  ...
  },
  {
    ...
    date: "2026-08-31",
    items: [
      "验收附件放开类型限制：不再限定 pdf/docx/xlsx/图片，任意常见文件均可上传",
      "验收附件旁显示上传人姓名（无实名信息时显示账号名）",
      "维保项目销售人员支持在“编辑基本信息”中手工修改或清空；负责人账号改派时可一并同步销售人员，项目卡显示最新姓名",
    ],
  },
  {
    version: "1.25.0",
  ...
  },
  {
    ...
    date: "2026-08-17",
    items: [
      "维保页面全新改版：原 22 个页面收敛为 2 个——「维保主页（项目卡墙）」与「项目面板」，信息更集中、路径更短；旧页面地址自动跳转到新主页",
      "项目卡墙：一行五卡展示项目名称、XSDD 合同号、项目经理、合同总额、维保期限与备件成本（含税/未税、缺失成本、维保备件采购数、回款预览）；成本÷合同额一条进度条三色提示——低于 80% 绿色、80–100% 黄色关注、超过 100% 红色报警；默认显示进行中项目，已结束与期限缺失可切换筛出，支持项目名/XSDD 单号模糊搜索，下滑自动加载",
      "项目期限与合同总额由系统既有数据自动确定，无需先导入台账：期限取自维保需求单的维保起止日期（覆盖 411/415 个项目），合同额按项目挂靠的销售订单自动汇总（覆盖 402/415）；销售订单被多个项目共用时标「共用单」、部分订单查不到金额时标「不完整」，一律如实标注不做猜测；日后台账导入将以台账为准自动覆盖",
      "项目面板：顶部出库明细可按合同筛选下钻到明细行；「项目基础信息 / 备件成本 / 报销 / 回款」四个页签即项目总表的网页呈现；基础信息页签内确认归属挂靠（候选按同 XSDD 销售订单预筛排最前）；点「编辑基本信息」可修改项目名称、维保负责人与维保期限，期限修改后进行中/已结束状态自动重算",
      "下载与上传共三处、在哪下载就在哪上传：主页全局下载全项目备件行级表（按今天/昨天/本周/本月/自定义时间段，改价补价后上传覆盖即为真实源）；项目面板下载本项目总表（六 sheet 一次下载整份上传）；各页签单独下载对应 sheet",
      "缺数据一律明示：未导入的数据源显示「尚未导入」、算不出的成本率显示「数据不足」，绝不显示 0 或用绿色冒充正常；发货单、返库单、入库单、回款等数据导入后对应板块自动点亮",
      "维保展示板整体由服务端总开关控制，异常时关闭开关即可整体回退，不影响已有数据",
    ],
  },
  {
    version: "1.21.0",
    date: "2026-08-12",
    items: [
      "新增维保管理正式工作台，并保留原项目数据、下载中心和项目提醒入口作为兼容路径。新增流程使用附加事实、影子状态、独立接口、实名系统账号权限与服务端停用开关；成本和库存口径切换另设默认关闭的独立停用开关",
      "新增项目方块工作台：项目卡内集中查看合同、回款、现场实际消耗、已审批报销、提醒与四表预览，并直接导出或回填 Excel；缺合同额、价格或期限的项目仍完整展示，只以小字提示待补字段",
      "合同额统一标注口径，并以两个进度条分别展示已回款/合同额和已消耗/合同额；成本达到合同额 80% 显示黄色关注，达到或超过 100% 显示红色报警，成本构成不使用扇形图",
      "新增维保需求单安全删除：可搜索、跨页选择并完整复核全部条目，必须填写删除理由、两次确认并等待服务端 7 秒；采用可恢复的逻辑删除和审计留痕，未完成正式口径切换前不改变兼容入口读数",
      "新增项目经理任务看板、每月全量工作簿和验收材料流程；现场备件领用单作为实际消耗事实但不直接写库存，并据标准品类生成坏件应返义务，硬盘自动免返，无应返项明确显示“无应返项”",
      "新增仓库单据预览、稳定关联与歧义处理工作台；只有已确认发货、唯一需求关系、当前项目归属和精确 PN 才进入现场领用候选，无法确定时留给人工处理，不按项目名或描述猜测",
      "新增销售经理补库购物车：按 PN 搜索并查看所属池和近半年采购/销售价量，填写数量与备注后提交；支持逐条审核反馈、打回复提、版本留存、人工审核 Excel 和维保需求单字段子集导出，不自动审批，也不修改库存",
    ],
  },
  {
    version: "1.20.0",
