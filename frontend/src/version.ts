...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    date: "2026-08-10",
    items: [
      "维保稳定版继续作为默认入口；新增独立 Beta 入口和醒目标识，实名白名单账号可主动进入并随时返回稳定版。两套页面共用同一后端和数据库，Beta 新增流程使用附加事实、影子状态、独立接口、权限与总闸；正式成本和库存切换另设默认关闭的独立闸门",
      ...
      "新增维保需求单安全删除：可搜索、跨页选择并完整复核全部条目，必须填写删除理由、两次确认并等待服务端 7 秒；采用可恢复的逻辑删除和审计留痕，Beta 试用期间不影响稳定版读数",
      ...
      "新增销售经理补库购物车 Beta：按 PN 搜索并查看所属池和近半年采购/销售价量，填写数量与备注后提交；支持逐条审核反馈、打回复提、版本留存、人工审核 Excel 和维保需求单字段子集导出，不自动审批，也不修改库存",
    ],
  },
  {
    version: "1.20.0",
