...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    date: "2026-07-04",
    items: [
      "项目追踪工作簿闭环：从系统导出的项目工作簿即是表单——报销明细页由员工续填，整本拖回导入页即可入库（系统只吃报销明细页，备件/预算等其余页自动跳过），导出格式=导入格式",
      "报销导入不再要求氚云导出（氚云无报销导出，口径更正）：任意来源的表格只要有 报销日期+报销金额 两列即可导入；行内没有销售订单列时按报销页第 1 行「销售订单」锚归集到项目；流程状态留空默认「已结束」计入已花",
      "报销防重升级：有单号按 单号+序号，无单号按内容（合同+日期+金额+事由+人员）自动防重——同一本工作簿每月更新后重复上传不会重复计费；「修复模式」= 以本表为准，本合同报销行整体替换",
