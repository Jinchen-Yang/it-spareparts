...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    date: "2026-07-14",
    items: [
      "修复互通池跨池旧响应：保存池 A 后切到池 B，A 的延迟响应不会再覆盖 B 的编辑基线，后续保存只会写当前池；详情与列表的过期响应同样会被丢弃",
      "收紧价格治理脱敏：无价格治理权限时，当前策略、历史备注、设置人和含/未税录入口径均不可见，不再能从策略元数据反推约束价",
      "收紧池数据安全：迁移遇到少于 2 个成员的历史池会中止并要求先治理；恢复单成员池会被拒绝；存在人工建池或池维护数据时 schema 降级会失败即停",
      "池管理接口新增安全白名单排序；创建或维护成员时，请求内重复 PN 会被明确拒绝，不再静默去重",
    ],
  },
  {
    version: "1.13.1",
