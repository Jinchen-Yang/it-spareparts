 ...
 */
export default function InlinePartEditModal({ open, pn, onClose, onSaved }: Props) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const { data } = await masterEdit(payload);
  ...
  return (
    <Modal
      ...
      okButtonProps={{ disabled: loading || !form || Boolean(loadError) }}
      width={560}
    >
