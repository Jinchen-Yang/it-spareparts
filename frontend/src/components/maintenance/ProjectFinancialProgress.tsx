...
export type CostWaterlineStatus = "normal" | "yellow" | "red" | "unknown" | "no_contract";
...
export function classifyCostWaterline({
  ...
  if (!costComplete) return { status: "unknown", percent };
...
function MetricProgress({
  ...
  const percent = numericPercent(numerator, denominator);
  return (
    <div data-testid={testId} style={{ minWidth: 0 }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, marginBottom: 4 }}>
        <span style={{ fontWeight: 600 }}>{label}</span>
        <span style={{ fontVariantNumeric: "tabular-nums" }}>{percentLabel(percent)}</span>
      </div>
      <Progress
        aria-label={label}
...
export default function ProjectFinancialProgress({ metrics }: {
  ...
  const sitePercent = numericPercent(
    metrics.site_requisition_known_cost,
    metrics.total_contract_amount,
  );
  const collectionPercent = numericPercent(metrics.received_amount, metrics.total_contract_amount);
      ...
      >
        ...
        {!metrics.cost_complete && (
          <div style={{ color: "var(--mb-warning)" }}>
            缺 {metrics.missing_cost_lines} 行成本；当前比例按已知成本计算，为下限。
          </div>
        )}
