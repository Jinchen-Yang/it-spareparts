...
## 十一、2026-08-24 解除 WBDD 导入「疑似不完整导出」50% 拦截（用户拍板）

- 用户原话：「上传改动多余 50% 就不予覆盖，这个限制先关掉，能 100% 全量修改。」
- 澄清：总表工作簿的 50% 行损失拦截已于 8-22（b20a305）撤除；本次是**另一处**——WBDD（维保需求单）导入后的"跟随氚云作废"链路：`_MISSING_SUSPICIOUS_RATIO=0.5`，缺失占比超 50% 即 `suspicious=true`，前端收起差异清单并禁用"按氚云现状批量作废"。
- 改动：后端不再输出 suspicious（missing_ratio 仍计算展示）；前端差异清单恒展开、批量作废不再按占比禁用。单次批量作废仍有 1000 张上限（MAX_DELETE_HEADERS，分批即可，非拦截）。误传风险兜底：作废确认弹窗 + 墓碑可恢复 + 审计。
- 测试：wbdd 导入/总表 v2 套件 61 过；前端 tsc + build + vitest 632/632 绿。
