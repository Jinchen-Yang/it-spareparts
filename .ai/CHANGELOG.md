...
## 2026-09-20 — v1.36.0 维保人工维护与页面批量（Release Candidate，待最终合流与验收）

**Agent:** Claude Code（仅版本元数据与发布说明）。
**Before:** 应用版本仍为 v1.35.0，需求 / 回款 / 领用 / 返还四类人工维护缺少统一的 v1.36 发布边界、并发语义、迁移链与生产闸门说明。
**After:** 前端 package 元数据与应用版本源升为 1.36.0；新增 `docs/releases/v1.36-manual-maintenance.md`，按处理对象区分页面单条、页面批量和 Excel 通道，记录逐行幂等 / OCC / 结果未知、返还 SN 与 16384 字符凭据、需求 override、`page_manual` 人工领用、迁移防丢守卫、预备备份、只读回滚与正式发布步骤。
**集成边界:** 当前 HEAD `305b36e` 已含需求单条前后端、回款单条及返还页面批量；需求页面批量、回款页面批量、人工领用仍在隔离工作树收口，明确标为待合流 / 待验收。当前迁移到 `d9a4c6e2f8b1`，人工领用合流后的目标 head 才是 `e7c1f9a3b5d2`。
**验证:** 本任务只执行 JSON 版本一致性检查与 `git diff --check`；未运行或宣称全量测试、Required CI、部署、生产迁移、真实登录或公网验收。最终证据由后续 freeze / PR / 生产发布回填。
**原因:** 为并行功能收口提供一个不越界、不混淆页面批量与 Excel、且不预支验收结论的 v1.36 发布候选说明。
**Commit:** 本条记录所在提交（最终 SHA 以 `git log` 为准）。
