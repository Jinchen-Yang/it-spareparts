...
## 2026-09-21 — v1.36.1 返还登记人姓名与 SN 展示

**Agent:** Codex。
**Before:** 登记人直接显示登录账号；凭据列未展示已保存的逐件 SN 或导入 SN。
**After:** 按当前页账号批量解析姓名，保留原始审计身份；SN 优先展示并保留前导零，原凭据仍可查看，无 SN 时回退凭据/单号。
**边界:** 仅调整读取与展示，不修改历史记录、数量或源凭据，不新增数据库迁移。
**发布记录:** 实际部署与验证以 v1.36.1 Release 为准。

---

## 2026-09-20 — v1.36.0 维保人工维护与页面批量

**Agent:** Claude Code 完成初始开发；Codex 接手审查整改、集成、测试及发布。
**Before:** 返还来源凭据仅 128 字符，四类业务的页面维护入口及批量流程不完整。
**After:** 凭据扩为 16384 字符；补齐需求与回款页面单条/批量维护、人工领用多明细维护、返还逐件 SN 与批量登记/修改/作废；保留 Excel 通道，逐行处理冲突与未知结果。
**集成边界:** 各开发提交已合流，迁移目标单 head 为 `e7c1f9a3b5d2`。旧版本只读回滚和生产备份恢复演练已准备，正式切换仍须 fresh backup。
**验证:** 完整前端 1037 项、类型/构建/依赖审计通过；四类业务真实页面、未知响应重试、16384 字符边界、生产备份恢复升级与旧版读取新 schema 均已验收。修复人工领用真实路由遮蔽并恢复坏件管理的高风险授权保护。完整后端以 [PR #333](https://github.com/Jinchen-Yang/it-spareparts/pull/333) Required CI 为准。
**发布记录:** 生产验收完成后在 [v1.36.0 Release](https://github.com/Jinchen-Yang/it-spareparts/releases/tag/v1.36.0) 记录精确 SHA、时间、最新备份和公网验收；发布前不提前标记为已上线。
**说明:** 能力及发布步骤见 `docs/releases/v1.36-manual-maintenance.md`；领用以一张单多明细维护，批量操作不承诺跨行数据库事务。

---

## 2026-09-17 — v1.35.0 开支按项目 / 销售主档口径 / 项目墙筛选 URL 化（本地待发布）

**Agent:** OpenCode（子智能体并行实现 + 主任务集成、隔离库浏览器验收）。
**Before:** 开支统计无项目维度；销售口径用订单源列，面板改过的销售不生效；项目墙筛选是组件态，进项目返回即丢。
**After:** 开支统计新增按项目汇总（可点入面板、未归属单列）；销售口径改为「有效销售」（项目主档优先，人工改过含清空即权威，空主档回退订单源，无项目用订单源），分组与筛选在两页签同步；项目墙筛选全量写入 URL，返回保持筛选。
**验证:** 后端 53 passed（含有效销售五情形与 by_project 恒等）；前端全量 83 文件 885 passed、tsc 与构建通过；隔离库浏览器实测王俊凯/王小环场景、按项目汇总、URL 直入与返回保持全部通过。详见 `docs/releases/v1.35-project-spend-salesperson-wall-url.md`。
**发布:** 无数据库迁移。已合并（`3ce1c6a`）并部署生产（2026-09-17），发布标签 `v1.35.0`；上线核验：迁移头 `e3a7b9c2d4f6` 一致、app 健康、`https://hbzgc.icu` 已服务 v1.35.0（按项目汇总 / 有效销售 / 墙 URL 化均已上线）。

---

## 2026-09-17 — v1.34.0 维保数据分析「开支统计」页签（本地待发布）

**Agent:** OpenCode（子智能体并行实现 + 主任务集成、隔离库浏览器验收）。
**Before:** 数据分析只有 PN 排名；时间维度只能选起止范围，业务类型与销售只能筛选，无法按期、按类、按人看开支。
**After:** 新增「开支统计」页签——按天/周/月/年分桶的堆叠柱状图 + 合计 KPI + 按业务类型汇总 + 按销售汇总 + 按期明细；业务类型拆分为整体维保/备件维保/算力运维/拆改配服务/非维保/未标注/未归属；销售可按名称筛选并直接看汇总占比；与 PN 排名共用全部筛选，粒度与筛选写入 URL。
**验证:** 后端专项与回归 44 passed；前端全量 82 文件 877 passed、tsc 与生产构建通过；隔离 QA 库浏览器实测粒度/口径/合计/销售筛选/业务类型联动/受限信封/重置，与同参数 API 逐个一致。详见 `docs/releases/v1.34-analytics-spend-trend.md`。
**发布:** 无数据库迁移。已合并（`77abfbc`）并部署生产（2026-09-17），发布标签 `v1.34.0`；部署前备份 `db-20260916-1817.dump`（校验通过）；上线核验：迁移头 `e3a7b9c2d4f6` 一致、app 健康、`https://hbzgc.icu` 已服务 v1.34.0（开支统计页签上线）。

---

## 2026-09-16 — v1.33.0 数据分析全字段筛选与「拆改配服务」业务类型（本地待发布）

**Agent:** OpenCode（子智能体并行实现 + 独立代码审查 + 主任务集成与浏览器验收）。
**Before:** 维保数据分析只有时间范围、业务类型、PN 搜索与排序；业务类型只有五档。
**After:** 新增项目、客户、销售、需求单号、需求类型、仓库、成本来源七个筛选，与既有条件叠加、写入 URL、重置一键清空；仓库候选来自实际出库数据并按行键范围收敛；业务类型新增第六档「拆改配服务」，维保主页筛选、项目面板编辑与数据分析同步。
**验证:** 后端相关回归 163 passed（analytics/boss board/角色范围/业务类型回填与写入）；前端全量 81 文件 859 passed、tsc 与生产构建通过；隔离 QA 库浏览器实测六档选项、七个筛选 KPI 与 API 一致、刷新恢复项目标签、主页卡片筛选与面板下拉。详见 `docs/releases/v1.33-analytics-filters-refit.md`。
**发布:** 无数据库迁移。已合并（`cbb586d`）并部署生产（2026-09-16），发布标签 `v1.33.0`；上线核验：生产迁移头 `e3a7b9c2d4f6` 一致、app 健康、`https://hbzgc.icu` 已服务 v1.33.0。

---

## 2026-09-16 — v1.32.0 需求单收货信息与分类成本分析（本地待发布）

**Agent:** OpenCode（并行实现、独立审查及主任务浏览器验收）。
**Before:** 两个需求单列表未展示已导入的收货信息；数据分析缺少项目业务类型筛选。
**After:** 按单展示地址、联系人和电话，服务端沿用客户信息权限；按当前项目业务类型筛选分析的需求明细及返还项目范围，汇总/排名/占比/分页一致，多选与刷新保留；修复重置筛选后搜索框残留旧关键词。
**验证:** 后端相关回归 186 passed；前端全量 80 文件、844 passed；类型检查与生产构建通过；隔离数据库实测两个入口、电话复制、权限脱敏、分类及合计、关键词叠加、空结果、刷新和重置，390px 筛选控件未溢出。详见 `docs/releases/v1.32-maintenance-contact-analytics.md`。
**发布:** 无数据库迁移；随 v1.33.0 一并发布（合并提交 `cbb586d`，部署 2026-09-16）。

---

## 2026-09-15 — v1.31.0 业务类型编辑与可折叠筛选

**Agent:** Codex。
**Commit:** `5bda77b`。
**Before:** 基本信息不能编辑业务类型；卡墙筛选始终展开，收窄条件缺少集中展示和逐项删除入口。
**After:** 新增五档业务类型编辑（未标注清空），保留 version/reason 审计和未编辑的历史值；期限导航常驻，筛选可折叠并记住状态；蓝色胶囊与角标反映已应用条件，支持键盘移除，导出/批量移交仍使用同一筛选。
**原因:** 按用户确认的 v1.31.0 改造方案，提高项目类型维护和卡墙筛选的清晰度。
**验证:** 前端全量 828 项通过，类型检查与构建通过；主页类型修正后专项 10 项通过；后端写入/回填/卡墙/导出专项 49 项通过。全量后端、浏览器和最新 CI 的最终结果以 PR 为准。
**发布:** 无后端或迁移改动。生产只读预览 618 候选、476 可填、0 冲突、142 无来源；尚未在本记录中声明已回填。流程与回退见 `docs/releases/v1.31-maintenance-filters-runbook.md`。

**浏览器补充:** `77c9d72` 收起时卸载筛选区域，消除下拉浮层残留；筛选值仍由父组件保留。修改后前端全量 828 项再次通过。真实浏览器验证编辑/清空、折叠记忆、键盘移除、筛选导出与无错误；XLSX 单行成本含税 226/未税 200 与夹具一致，类型变更审计可查；隔离回填只填唯一来源且重复执行为 0，冲突与无来源保持空值。
...
**Commit:** `489fa67`。
**Before:** 同时选择业务类型与回款生命周期时，隐藏计数复用了已按业务类型缩窄的回款项目集合；“回款完成”少算隐藏项目，其他期限桶多算。
**After:** 回款候选仅移除业务类型限制，保留权限、搜索与生命周期；一次批量回款集合同时支持主列表和扩大范围的隐藏计数，查询次数不随项目数增长，无财务权限不计算该集合。
**原因:** 修复 [PR #323](https://github.com/Jinchen-Yang/it-spareparts/pull/323) 的自动审查 P2，确保隐藏数量等于同条件下取消业务类型筛选后多出的项目数。
**验证:** 新增回归先复现8项失败；修复后相关专项 **92 passed / 0 failed，53.90秒**，覆盖四桶、多选/分页/空页、权限/关键词/活动/归档及固定批量查询。日志 `/tmp/pr323-business-type-red.log`、`/tmp/pr323-business-type-green.log`，`git diff --check`通过。下文4482项全量属于修复前代码，最终验收以PR最新head的远端CI为准。
**状态文字校正:** 实施计划 §4.3 不再把已通过的 npm 在线审计写成待授权；交接明确各轮全量对应的代码版本与PR最新检查入口。演示刷新至审查修复（PID3853749），8表全部字段hash和5条收货不变，24组只读筛选检查与浏览器验收通过。此项仅校正文档状态，不改变业务口径。
**依赖补充:** 在线生产依赖审计通过；全依赖中的两个 moderate 对应开发工具 Vitest 同一公告 GHSA-82fw-gwwq-j7x9。现有测试/生产配置无公告所述开发服务器入口，后续独立升级兼容验证；未将全依赖记为无漏洞，PR说明已披露。
...
- `e98ba71`：交接、接口和已确认业务规则文档。
- `f115fe2`：回填最终全量通过、准确跳过项及当时远端交付状态。
- `8e69e75`：记录按用户后续指令完成的前端在线依赖审计。
...
- 后端生产依赖审计无已知漏洞。用户在明确得知npm元数据发送阻塞后指示“你解决一下”，已按该授权执行前端在线 `npm run audit:prod`：exit0，`PRODUCTION_AUDIT_OK: no known runtime vulnerabilities`，日志 `/tmp/return-ledger-frontend-audit-online.log`。此前自动审批拒绝已因后续明确指令解决；离线audit会跳过，未作为验证证据。

**交付与环境:**

- 本轮保留接手时19笔既有集成历史，新增按后端/前端/回归分组的提交；计划原分阶段PR调整为完整integration PR，避免把未独立验证的旧前置树当作已验收版本。只以最终树作为交付候选。
- 完整交付PR为 [#323](https://github.com/Jinchen-Yang/it-spareparts/pull/323)；其远端检查、审批和最终合并状态以PR及远端main为准。创建PR时main=e0c80b0且protected=true，两个必需CI检查已启动；详细保护端点无读取权，不能推断审批要求已取消。PR321等价tree已核实，是否合入主干以#323的最终合并结果为准。
