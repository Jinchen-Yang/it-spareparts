...
- 状态：用户已于 2026-08-14 确认，进入实施；生产发布仍受第 12 节门禁约束
- 日期：2026-08-14
- 设计评审基线：`c431656bd2615102f053199801554191b2d88791`
- 主工作区冻结实施起点：`eae82716329ac9ebfecf33c04c630931e240a46b`；该 merge 已完成早期 `d7168ce3` 工作线向设计评审基线 `c431656b` 的对齐，禁止再次执行旧对齐流程
...
| `operation_key` | 只由实名操作者和客户端 `Idempotency-Key` 生成的预览幂等键；命中后再比较现有 `file_sha256` 与 `contract_version`，任一不同即冲突 |
...
- 主工作区实施起点已冻结为 `eae82716329ac9ebfecf33c04c630931e240a46b`；该提交已将早期 `d7168ce3` 工作线对齐到已复核的 `c431656b`，并保留 `.ai`、未复活远端已删除的旧氚云项目导入链。当前更新的 `.ai` 执行合同仍须先独立提交并建立新 `run_id`，业务代码才能开工。
