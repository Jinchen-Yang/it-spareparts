...
待办全部由系统规则生成，项目经理不能凭空新建或删除；可以接收、转派、补充处理说明和证据并提交系统复核。源事实修复后由系统自动关闭，确需忽略时必须填写原因并保留审计。任务唯一键为 `rule_code + project_id + source_id + reporting_period`，同一问题持续存在时更新原任务，不重复生成。

首批自动规则至少覆盖：月度全量表逾期、上传校验或版本冲突、现场领用单无法关联项目、领用成本缺失、成本率达到黄色或红色阈值、合同/领用/报销关键字段缺失，以及审批完成的报销无法关联项目。
...
- 费用报销支付单归集差旅、服务和其他备件以外支出；只有流程已经审批完成的报销单进入项目成本，其他状态只保留事实。表头报销金额按费用单号去重，不能和备用金、借款、核销、实付或明细合计重复相加。
...
- 项目经理每月下载系统当前全量工作簿，在表尾追加新的回款等记录，再将整本工作簿上传回系统；这不是仅上传当月增量。
- 已有行使用稳定实体 ID、基础版本和行签名定位；完全相同的已有行不重复累计。删除 Excel 行不代表删除系统事实，历史金额更正必须显式发起并保留原值、新值、原因和操作者。
- 上传前先展示新增、变更、缺失、冲突和拒绝行。上传基于旧版本或任一关键校验失败时整本零写入，不能静默覆盖后来版本。
...
- 四个可见 Sheet 固定为：`01_总览`、`02_备件消耗`、`03_报销单`、`04_项目经理追踪与提醒`。协议、行签名、版本和字典可以使用隐藏技术 Sheet，不计入业务可见四表。
...
- 旧 `/maintenance/downloads` 路由保留兼容跳转，进入项目工作台并打开导出面板；旧域名跳转到 `hbzgc.icu` 时保留原路径和查询参数。
...
| 提交项目费用 | 报销人员 | 财务、项目经理 | 关联订单、费用类别、事由、日期、金额、流程状态和附件 | 审批完成后进入项目费用，其他状态只保留事实 |
...
| 前置库、现场领用、返还义务和正式入库尚未闭环 | 实际消耗已确认以现场领用单为准；当前维护数据与成本能力不能替代实物生命周期 | 先实现领用事实与成本；返件事实保留但首版不冲成本 |
| 费用生效状态已有新口径，项目映射与日期仍需落契约 | 只有审批完成的报销单进入成本；尚需在模板中锁定精确状态原值和项目关联键 | 在导入规格中失败关闭未知状态，不从中文近义词猜测 |
| 当前下载中心与项目面板分离 | 用户无法在下载前确认范围、Sheet、行数和缺失数据 | 改为项目内导出预览，页面与工作簿共用读模型 |
| 现有 DEV-15 成本参考禁止未来前视 | 新口径明确允许以领用日为中心前后 7 天采购/销售加权 | 仅在新实现 Issue 中修改算法、测试和历史重算边界；本手册记录目标口径，不冒充当前能力 |
...
7. 费用报销到项目唯一编号的映射、审批完成状态原值和归属日期；
8. 巡检、盘点等非数据质量任务的周期、责任人与关闭证据；
9. LangGraph AI 自动审核的规则、阈值、所需证据、审计记录和标准退回原因。
