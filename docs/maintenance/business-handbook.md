...
| 生产尚未包含 #200 | 当前生产 SHA 为 `ab42005b`，并且生产目录有两处未纳入 Git HEAD、来源待解释的运维脚本 tracked 改动 | 发布前先解释并保护生产差异，锁定备份和回滚 SHA；部署、数据迁移和真实账号验收分别放行 |
...
1. **代码审核门**：固定 #200 的 base/head 和合并 SHA，确认 PR CI 与合并后 `main` CI 全绿，完成独立 Standards、Spec 和产品数据就绪审核。
...
4. **生产发布门**：解释并保护生产目录已有差异，完成数据库与应用备份，锁定目标 SHA、迁移版本、回滚 SHA 和回滚步骤，再取得单独部署授权。
