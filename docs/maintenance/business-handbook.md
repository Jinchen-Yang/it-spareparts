...
| 元数据 | 内容 |
|---|---|
| 状态 | 已确认业务口径；已由 #244 合并到 `main`，为正式口径 |
| 业务负责人 | 甲方维保业务负责人（文档不记录个人姓名） |
| 维护人 | IT_data PM/BA 与发布总控 |
| 最近核对日期 | 2026-08-12 |
| 适用范围 | 维保合同履约、备件流转、项目费用、维保负责人待办及相关数据治理 |
| 关联 Issue | [#128 维保业务工作台总控](https://github.com/Jinchen-Yang/it-spareparts/issues/128), [#193 业务手册](https://github.com/Jinchen-Yang/it-spareparts/issues/193) |
| 关联 PR | #244（已合并 main@caf4a973），后续开发见 [#128](https://github.com/Jinchen-Yang/it-spareparts/issues/128) 依赖图 |

> 本手册是维保业务正式口径。原始会议记录、聊天记录和字段分析只用于追溯来源，不与本手册形成第二份可编辑真相。
...
8. 维保负责人每月下载当前全量工作簿、在表尾追加新记录并回填上传；系统比较版本并生成回款、成本和数据完整性待办。
...
| 维保负责人待办 | 告诉负责人下一步必须处理什么 | 系统规则及合同、回款、领用、费用和数据异常 | 处理证据、系统复核、自动关闭或继续逾期 |
...
- 维保负责人只需以维保备件需求单发起业务需求，不重复填写第二张发货申请。
...
- 补库 Beta 与稳定版共用数据库基础设施，但使用独立业务表、API、页面、权限和服务端总闸；稳定版默认可用，Beta 仅对白名单实名账号开放，并始终提供返回稳定版入口。首批维保只读账号必须至少绑定一个未停用的有效主负责人项目，补库创建账号必须是 `sales` 角色并通过精确版本 canary。

### 3.5 维保负责人工作台

维保负责人登录后应优先看到自己负责项目的行动项，而不是先看无行动意义的提醒总览：
...
已确认边界是待办全部由系统规则生成，维保负责人不能手工新建业务待办。待办的接收、转派、处理说明、复核、忽略、自动关闭和去重键属于实现建议，需在待办开发 Issue 中单独确认，不能由研发把建议直接当成已确认流程。
...
- 维保负责人每月下载系统当前全量工作簿，在 `01_总览` Sheet 的固定“回款明细”表尾追加新记录，再将整本工作簿上传回系统；这不是仅上传当月增量。该 Sheet 同时包含只读“合同清单”，让每笔回款能关联并核对所属合同。
...
- 四个可见 Sheet 固定为：`01_总览`、`02_备件消耗`、`03_报销单`、`04_维保负责人追踪与提醒`。`01_总览` 至少包含只读合同清单和可追加回款明细两个固定表。协议、行签名、版本和字典可以使用隐藏技术 Sheet，不计入业务可见四表。
...
|---|---|---|---|---|
| 建立维保项目 | 商务或项目负责人 | 维保负责人、财务、仓库 | 合同、期限、负责人、服务范围、合同金额 | 项目主档和周期计划 |
| 更新合同与回款 | 维保负责人 | 系统、财务 | 当前全量工作簿、稳定行 ID、全部合同和新增回款 | 差异预检、版本化导入、回款进度和逾期待办 |
...
| 仓库供货 | 仓库 | 维保负责人、现场人员 | 需求单、仓库、PN/SN、数量、出库和发货信息 | 发货/出库单与在途状态 |
| 项目现场收货 | 维保负责人或现场人员 | 仓库 | 实收数量、SN、差异和证据 | 前置库增加或差异待办 |
| 现场领用 | 现场人员 | 维保负责人、仓库 | 已确认现场领用单、项目、设备、PN/SN、数量、时间、返还要求 | 前置库减少、消耗事实、成本取价和返还义务 |
| 发起返还 | 现场人员或维保负责人 | 仓库 | 项目、需求单、PN/SN、数量、原因 | 退货返库单和退返入库通知 |
| 返还件验收入库 | 仓库 | 维保负责人、库存系统 | 检测结果、入库仓库、库位、数量 | 可用库存恢复或维修边界交接 |
| 提交项目费用 | 报销人员 | 财务、维保负责人 | 关联订单、费用类别、事由、日期、金额、流程状态和附件 | 审批完成后具备计入资格；正式金额字段待样板确认 |
| 项目结束 | 维保负责人 | 仓库、财务、管理层 | 剩余备件、未结费用、未收款、验收材料 | 剩余备件返库并关闭项目事项 |

## 5. 当前能力分层

以下状态截至 2026-08-12。`main` 上 #244（caf4a973）已合并，包含 #201、#203—#210 的主要实现。

### 5.1 GitHub `main` 已具备（#244 交付）
...
- **（#244 新增）** 稳定项目主档、维保负责人归属和来源单精确指派；
- **（#244 新增）** 维保需求单搜索、跨页选择、逻辑删除墓碑、理由、两次确认和服务端 7 秒闸门；
- **（#244 新增）** 维保负责人本人任务看板、月度全量工作簿和验收报告提交/复核；
- **（#244 新增）** 现场领用事实、成本证据冻结、坏件应返义务、标准品类硬盘自动免返和仓库确认试算返还率；官方返还率分子仍待业务确认；
- **（#244 新增）** 仓库文件零写预览、稳定事实入库、歧义工作台，以及已确认发货到现场领用候选的精确投影；
- **（#244 新增）** 成本/库存迁移 dry-run、证据对账、职责分离审批和哈希绑定 manifest；生产切换总闸仍为关闭；
- **（#244 新增）** 销售经理补库购物车 Beta、独立白名单权限、半年价量证据、逐条审核反馈、打回复提、版本留存、人工审核 Excel 和 WBDD 字段子集导出；Beta 总闸仍为关闭。

`main` 中存在这些代码不自动证明生产已经部署同一 SHA。

### 5.2 待开发（#128 维保业务工作台重构计划）

以下能力由 [#128](https://github.com/Jinchen-Yang/it-spareparts/issues/128) 统一规划，不在本手册详细展开：

- **前端业务化**：可折叠导航、正式路由、维保负责人工作台、五阶段项目工作流、业务文案统一；
- **权限隔离**：维保需求单按项目负责人范围隔离；
- **采购链展示**：采购订单→维保需求单→项目的只读关联视图；
- **氚云同步**：项目/合同/回款节点的受控导入、预检与审计；
- **三类账号全流程验收**：admin 全部、维保负责人本人、无项目账号零项目。

详细依赖图见 [#128](https://github.com/Jinchen-Yang/it-spareparts/issues/128)。仓库真实模板接入保持独立 P1 Issue，不与前端业务化互相阻塞。
...
| 现场领用取价切换尚未生产启用 | 新口径允许领用日前后 7 天采购/销售数量加权，但历史基线和真实样本仍需对账 | #210 独立审核尚有 P1，即使已产生 manifest 也不得切换；缺价永不按 0 |
| 新旧报销仍缺逐条双真值对账 | 旧 BXD 和新已审批费用目前只比较汇总金额，不能证明稳定明细没有丢失、重复或改归属 | 按费用稳定 ID、日期、金额和合同归属生成 parity blocker；完成前禁止正式迁移审批 |
| 历史成本基线还不是服务端工件事实 | 现有页面可填金额、行数、定位和哈希，只能证明输入内部自洽，不能证明它们来自某个真实完整文件 | 只接收服务端 artifact/import ID，由服务端重算 SHA-256、行数、覆盖期和金额；禁止保存带凭证 query 的 URL |
| 仓库完整水位和旧文件覆盖未形成机器证明 | 人工日期或当前库内已有行数无法证明某月文件没有漏导；空数据也不能直接解释为零变动 | 绑定成功导入批次、预期文件集、连续覆盖期和文件哈希；缺批次或断档失败关闭 |
| 迁移重算性能和锁影响未验收 | 大项目可能在持有事实表 SHARE 锁时读取大量样本并逐行过滤，会影响正常导入 | 改用一致性快照、按 PN/日期分块和 SQL 层限额，完成高基数与并发锁演练后再开放 |
