...
| 项目总表工作簿（V2） | 业务手工维护 Excel（原 AB-3 `04_报销订单`＋`05_项目经理回款单` 两张 sheet 已扩为 V2） | `01_项目基础信息`（展示）＋ `02_回款计划` / `03_备件明细` / `04_费用报销` / `05_实收回款` / `06_领用返还` 五张可编辑表**一本多 sheet**，下载 → 本地增删改 → 上传；显式 CREATE / UPDATE / VOID，删行 = 作废（D-01），只校验用户改过的行、不整本拒绝（D-02），修改权按 D-03 下放 |
...
- 需求单规则（F1）：全量快照 upsert；同文件重传幂等；上批有本批无 → 进对账差异清单，导入本身
  **不删除不隐藏**；差异清单页与需求单列表可对整单**一键作废**（D-12，`POST /maintenance/demands/void-fast`：
  逻辑墓碑＋挂靠停用，管理员实名恢复）；源单状态列的已取消/作废保留入库、原样展示、聚合不剔除（系统内墓碑另计，见 D-12）。
  **修改通道**：需求单源头字段仍走氚云改后重传覆盖；备件行的补价/改价/作废走项目总表 `03_备件明细` 的显式
  CREATE / UPDATE / VOID（D-01 删行 = 作废、D-03 修改权下放），不再是「系统内不手工编辑」。
...
- **②项目面板**：出库明细（按时间筛选；多合同并排或筛选）＋子 tab（当前 6 个：概览/备件与需求单/报销/
  回款/领用与返还/验收＝项目总表的 web 呈现）＋下载本项目总表（多 sheet）＋各 tab 下载对应单 sheet；项目名旁编辑基本信息。
