...
不能为减少 smoke 参数而互相跨域授权。reader 账号必须满足 `page_maintenance=true`、
`page_maintenance_beta=true`、全部 Maintenance 写 action=false，且全部 Replenishment 权限为
false。create 账号必须满足 `page_maintenance_beta=false`、`page_replenishment_beta=true`、
...
严格证明稳定维保读取与 Maintenance Beta GET=200，同时确认不能访问 Replenishment；creator
的页面、价格与 create 均为 true，Maintenance Beta 与 review 为 false，并且不存在任何
...
- 空白名单 deny、目标实名原始权限图摘要、Maintenance write count=0、admin pilot count=0、
  Maintenance reader/creator/review 计数、Replenishment create canary、reader/creator/deny smoke；
- 0/5/15/30 四份观察结果；
