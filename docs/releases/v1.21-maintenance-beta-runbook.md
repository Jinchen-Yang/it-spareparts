...
两名独立审核者必须分别产出 `exact-sha-independent-review-v1` JSON，绑定同一
`TARGET_SHA`，并包含 `scope: full-release-candidate`、`p0_count: 0`、`p1_count: 0`、
`conclusion: approved`、reviewer id 和完成时间。生成器要求至少两份、reviewer id 不同；
不能用 PR 评论摘要代替 exact-SHA 终审证据。
...
`v121-action-canary-v1` JSON。证据必须绑定 `username`、`action`、`target_sha`、
`environment: isolated`、实名执行人、带时区完成时间、HTTP 写方法/API 路径、请求 payload
摘要、预期/实际 2xx 状态码、响应摘要及 `conclusion: passed`。allowlist 通过路径和 SHA-256
绑定该文件。manifest 只保留整张
...
脚本设为 `700 root:root`；发布执行器会逐项拒绝宽松权限、symlink 与 hardlink。
...
安全阶段通过后，才由权限中心逐账号写入 manifest 中的权限图。权限图还必须纳入所有
action-only Replenishment reviewer；例如当前实现中实名 admin 的 review action 若有效，
即使没有 Beta page 也必须列入并提供 canary 证据，否则摘要不匹配、总闸不能打开。泛化
`admin`/共享账号不符合实名 pilot，存在时本次 Replenishment Beta 保持关闭并先整改账号。
发布脚本按运行时的 admin invariant 计算权限：所有活跃 admin 都视为拥有 review callback，
不会因为数据库快照中的 action 位为 false 而漏出摘要；任一活跃账号缺少有效权限快照也会
直接拒绝开闸。
...
同时打开两个总闸、recreate app，并证明“名单外拒绝”和“名单内允许”。
...
配置变化后脚本必定 recreate app；若无法证明新配置已经生效且恢复健康，脚本会停止 app，
避免仍带着旧的开放环境变量继续服务，再转人工 forward fix。若当前已经回切到演练通过的
