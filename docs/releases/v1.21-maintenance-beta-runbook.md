...
| Replenishment create/review | 仅允许 exact-SHA、GitHub 在线复验通过的真实 canary；create 另需价格权限，review-only 不需价格权限 |
...
9. Replenishment 可使用独立实名白名单，但必须同时记录价格权限、create 和 review
   action；create/review 只有绑定 exact target SHA 且生成 manifest 时经 GitHub API 再次
   在线核验的真实 canary 证据才能为 `true`。
...
的原始运行时 effective 值。首批任何账号都不能是 admin；所有 Maintenance 写 action 必须
逐项为 `false`，包括工作簿回填、项目管理、批量删除、现场领用、坏件返还、验收、仓库应用和
`action_maintenance_migration_review`。提供 Maintenance canary 也不会放宽这一限制。

只有 Replenishment 的 `action_replenishment_create` / `action_replenishment_review` 可在本次
设为 `true`，且必须先在 exact target SHA 的隔离 canary 中真实执行。执行人随后用自己的
...
Replenishment create/review 实际使用的 canary 会按“多余证据”拒绝，不能预埋未来权限。

最小权限必须保持：create 账号需要 `page_replenishment_beta=true`、
`data_pool_price_governance=true` 和 create canary；review-only 账号需要页面位、review action
和 review canary，但允许 `data_pool_price_governance=false`，目录接口应返回 403。不得为了让
审核回写通过而额外授予半年采购/销售价格。
...
cutover flag=false、Replenishment writes 仅接受在线 canary，同时保留 schema migration 为
必需前置。输出目录已存在时生成器拒绝覆盖。
...
先只打开 Maintenance 全局闸并保持页面白名单为空。Replenishment 的审核回写是
action-gated callback，不依赖页面白名单，因此空名单阶段必须保持其全局闸为 false：
...
Replenishment review 也必须经过 `page_replenishment_beta` 的实名白名单，不再存在 action-only
admin 例外；审核账号必须是非 admin，并显式持有 `action_replenishment_review`，再以 exact-SHA
canary 证明。若最终候选 SHA 中 review route 缺少页面白名单依赖，静态门禁和独立审核必须阻断
发布。review-only 账号无需价格权限；生产 allow smoke 必须证明 capabilities 可读、
`can_review=true`、`can_view_price=false` 且 catalog=403。任何活跃账号缺少有效权限快照同样
直接拒绝开闸。
...
Replenishment 投影摘要，只输出摘要，不输出用户名；它们必须逐项等于 manifest。随后才
同时打开两个总闸、recreate app，并证明“名单外拒绝”和“名单内允许”。pilot 基线只接受
...
- 空白名单 deny、目标实名原始权限图摘要、Maintenance write count=0、admin pilot count=0、
  Replenishment canary、allow/deny smoke；
- 0/5/15/30 四份观察结果；
