...
这里的 Maintenance“只读”精确定义为**业务事实只读**：Pilot 不得新增、修改、删除项目、
领用、返还、验收、成本或库存事实。工作簿 `validate` 可以写入隔离的控制态 `validation batch`
与校验问题清单，用来复核导入内容，但不能 apply 到业务事实；对应 apply/action 仍固定为
`false`。因此不能把一次 validate 的 HTTP 200 误报成业务写权限已经开放。
...
python3 "$REPO_ROOT/.deploy/v121_beta_static_test.py"
...
该静态自检位于现有 required 后端 check“后端测试（pytest + 迁移链验证）”内，不新增第三个
required check，避免分支保护配置与 manifest 门禁漂移。它只运行非生产静态/隔离测试，不能
代替 exact-SHA CI、生产快照或真实账号 smoke。
...
不能为减少 smoke 参数而互相跨域授权。reader 账号必须同时满足 active project + active `primary_manager`
两层资格：项目主档仍有效，且至少存在一条未归档的主负责人关系；随后还必须满足 `page_maintenance=true`、
...
每个 allowlist 账号还必须提供 `runtime_permissions`：键集合逐字等于
`backend.app.permissions.ALL_KEYS`，包含全部 `data_*`、`page_*`、`action_*` 与
`own_customers_only`。manifest 额外固定 `runtime_effective_permissions_sha256`；生产开闸前、
全部 smoke 后和每个观察点都从 `template_perms ⊕ perm_overrides` 重算。这样
`data_customer`、`data_purchase_cost`、`data_profit` 或行级范围变化也会关闸，不能只靠 Beta
页面/action 子集摘要漏过可见范围漂移。
...
迁移前会在停掉公开面后再次要求 blocked session、长事务和 waiting lock 全为零，并在运行
Alembic **之前**持久化 `migration_in_progress` 与 `database_restore_forbidden=true`。从该点到
`migrated` 持久化完成之间，`ERR/EXIT/HUP/INT/TERM`、采样异常、未知返回或非预期 DB head
统一写为 `migration_uncertain`，保持两道 Beta flag=false 并 stop/kill/反查公开面。`contain`
若看到中间 revision 或上述不确定状态，绝不执行 Compose up，只允许人工 forward verify/fix。
...
凭据必须是 `600 root:root`、link count=1 的普通文件；脚本用 `O_NOFOLLOW` 打开后在同一个 fd
上执行 `fstat` 与 JSON 读取，拒绝 symlink/hardlink 和检查后换文件。Compose up 之后任一
容器身份、running、health、stable smoke 或 deny smoke 失败，脚本都会重新持久化三 flag=false，
stop/kill 并反查 app/frontend，再写 `contained_after_failed_deploy`；不会留下半上线公开面。
...
`open-empty-beta` 与 `pilot-smoke` 从首次开闸写入前开始持有故障事务：任何
`ERR/EXIT/HUP/INT/TERM`、`.env` rename 后目录 fsync 失败、recreate/smoke/资格复验失败都会
幂等关闸并恢复稳定面；恢复失败则 stop/kill/反查公开面。只有 deny/reader/creator smoke、
第二次完整资格摘要和 `pilot_open` durable state 全部完成后才解除该收口 trap。
...
除证明稳定维保读取与 Maintenance Beta GET=200 外，还必须以 `owner_scope=me` 且
`include_inactive=false` 读取到至少一个本人负责的有效项目，并确认不能访问 Replenishment；
creator 必须保持精确 `sales` 角色，其
...
## 15. #240 发布控制修复追踪（2026-08-12）

- Before：deploy 在 Compose up 后直接 `fatal`；开闸/迁移缺少完整信号事务；迁移前没有 durable
  in-progress 状态；权限摘要遗漏非 Beta 数据/行权限；凭据校验存在 path 再打开窗口；
  BadReturnPanel 测试遗留 AntD message portal。
- After：上述路径均失败关闭，完整 `ALL_KEYS` 图进入 manifest/live 摘要，静态自检纳入原后端
  required check，前端测试生命周期销毁 message 单例。
- 验证：`python3 .deploy/v121_beta_static_test.py`、三个脚本 `bash -n`/ShellCheck、
  BadReturnPanel 相邻回归与整文件 Vitest；结果与 RED/GREEN 摘要记录于交付 ledger。
- Commit anchor：`#240` 对应本分支最终修复提交；exact commit SHA 以 Git 提交与交付 ledger 为准。
