...
首发清单至少包含一名 Maintenance 只读账号和一名 create 账号；两者可以是同一个实名账号。
create 账号必须满足 `page_replenishment_beta=true`、
...
首发生产凭据必须是 manifest 中的实名 creator：页面、价格与 create 均为 true，review 为
false，并且不存在任何 Maintenance 写 action。生产 creator smoke 必须证明 capabilities 可读、
`can_view_price=true`、`can_create=true`、`can_review=false` 且 catalog=200；Maintenance-only、
只读 Replenishment 或错误带 review 权限的账号都不能替代 creator 通过闸门。任何活跃账号缺少
有效权限快照同样直接拒绝开闸。
权限变更会吊销旧 token；操作员重新登录后执行：

```bash
sudo "$PACKAGE/v121_beta_release.sh" "$PACKAGE" "$EVIDENCE" pilot-smoke \
  /root/beta-denied-credential.json /root/beta-creator-credential.json
```

脚本先在两个总闸仍为 false 时从 DB 计算账号数、完整有效权限图、Maintenance /
Replenishment 投影摘要，只输出摘要，不输出用户名；它们必须逐项等于 manifest。随后才
同时打开两个总闸、recreate app，并证明“名单外拒绝”和“实名 creator 可用且 review 关闭”。
pilot 基线只接受
...
每个点都重跑稳定版读取、Beta deny、creator profile/capabilities/catalog、HTTPS/DB health、DB container identity、
...
- 空白名单 deny、目标实名原始权限图摘要、Maintenance write count=0、admin pilot count=0、
  Maintenance reader/creator/review 计数、Replenishment create canary、creator/deny smoke；
- 0/5/15/30 四份观察结果；
