...
```json
{"format":"v121-independent-review-attestation-v1","target_sha":"由审核者填写实时 TARGET_SHA","scope":"full-release-candidate","p0_count":0,"p1_count":0,"conclusion":"approved","report_url":"该仓库内审核报告的 GitHub HTTPS URL"}
```
...
再次在线查询并逐字段比对，手写 JSON 或 PR 评论摘要都不能过闸。
...
任何写 action 改为 `true` 前，先在 exact target SHA 的隔离 canary 中验收。执行人随后用
自己的 GitHub 实名协作者账号在 exact commit 下提交未编辑的 JSON commit comment，字段必须
...
拿另一个成功接口冒充会被拒绝；其中固定工作簿回填必须实际命中
`POST /api/maintenance/roundtrip-import`，不能用共用权限下的集合新增接口代替。allowlist 通过路径和 SHA-256 绑定捕获文件。manifest 只保留整张
