...
for raw in pathlib.Path(env_path).read_text(encoding="utf-8").splitlines():
    ...
    if key in wanted:
        ...
        env[key]=parsed[0]
if wanted-env.keys(): raise SystemExit("protected env key missing")
...
preflight 会重新确认：manifest 全部 artifact 摘要、当前 Compose、旧/新镜像、DB 容器与
镜像、DB head=f1、restart count、三 flag=false、HMAC id/fingerprint、无活跃导入、无
