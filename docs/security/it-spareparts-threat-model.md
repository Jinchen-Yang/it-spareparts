...
12. A future Human Template contains an unbounded regular expression, expression language payload, or instruction-like cell → cleaning worker evaluates it with broad capabilities → CPU is exhausted or data/files are accessed outside the declared patch language.
...
| TM-006 | Authenticated or remote abusive client | Can upload/chat repeatedly or create costly plans | Exhausts upload memory, DB pool, storage, model tokens, GPU queue, regex evaluator, or workflow retries | Main application outage and cost spike | API, DB, storage, GPU availability | Chat limits and max tool iterations exist (`backend/app/api/agent.py`, `backend/app/config.py`, `backend/app/agent/runtime.py`) | Whole-file upload read; no unified per-user task/file/query/token budgets or durable queue | Streaming upload cap; per-user/role concurrency; plan step/time/token limits; regex timeout; circuit breakers; separate worker pools; storage quotas | Queue depth, p95 latency, DB pool saturation, bytes/user, token/GPU seconds/user | high | medium to high | high |
...
| TM-013 | Malicious or mistaken Human Template author | Template cleaning workflow supports regex, expressions, mappings, or instruction-like content | Causes ReDoS, expression injection, path/field escape, or policy override | Availability loss, wrong files, or unauthorized data access | Templates, files, workflow integrity | Workflow is not yet implemented; target design treats cells as data (`docs/agent-platform/read-only-agent-domain.md`) | No constrained template DSL/schema, regex timeout, signer/version policy, or evaluator sandbox | Use declarative allowlisted operations only; JSON schema; bounded regex engine/timeouts; no eval; signed/versioned template; dry-run diff and human confirmation | Template publish audit, timeout/reject counters, diff anomaly alerts | medium | high | high deployment blocker |
