...
- 运行时 URL fetch、网页搜索、Shell、SQL、动态代码或环境读取指令。
...
每个候选生成不可变、机器可验证的 `skill-admission/v1`：
...
- 两名 Reviewer 必须是不同实名主体，候选作者不能自批；两人签署同一个
  `bundle.sha256 + manifest_sha256`。
...
- 数据库 DSN、网络地址、URL、重定向目标或“把结果上传/回传”指令。

允许 Markdown 出现文档链接不等于允许运行时访问；所有外链都进入清单，检查 scheme/host/重定向
风险并由 Reviewer 判断。Skill runtime 没有 URL fetch Capability。
...
- approved bundle 内容寻址并签名，随应用镜像/发布包进入生产；生产不保留 Git 凭据或安装器。
- `ENABLE_THIRD_PARTY_SKILLS` 默认 false。只有部署清单显式声明的
  `skill_id/version/bundle_sha256` 可加载。
- 启动时校验签名、Manifest canonical hash、bundle hash、逐文件 hash、status 和 policy version；
...
生产不得存在 install/upload/import Skill API，也不得在聊天中解释 GitHub URL 为安装命令。未知
...
- 启动及每次 Planner/Step 前验证 exact bundle、状态、权限、Capability 和 policy fingerprint。
