...
```json
{
  ...
  "input": {
    ...
    "source_application_ref": "optional external reference",
    "pn": "requested PN",
    "requested_qty": "5.000",
    ...
    "device_ref": "optional opaque reference",
    "fault_ref": "optional opaque reference",
...
- `source_application_ref` 最长 64，`target_warehouse` 最长 64，`justification` 最长 1000。
- 不接受客户端提供的 `as_of`、申请人、角色、阈值、规则版本或结论。
- `as_of` 在 Task 创建时按 `Asia/Shanghai` 日期固化，恢复执行时不得漂移。
- `project_contract_id` 存在时必须属于给定 `project_id`。
- `project_id/project_contract_id` 可以在系统内核验；`device_ref/fault_ref` 只能标记为
  `applicant_declared, verified=false`，不能伪装为系统事实。
...
- `collect_commercial_history`：固定查询半年采购/销售事实。
- `evaluate_hard_gate`：只执行不可覆盖的半年门槛。
...
- `seal_evidence_package`：固化 Evidence、规则版本和 HMAC fingerprint。
...
审计事实源。查询错误进入 #223 的 retry/fail 路径，不能创建“零记录”Evidence。
...
未来日期、空日期、非正数量、取消/未生效单不计入，并分别输出数据质量计数。Evidence 同时记录
采购/销售最新事实日期和最近成功导入时间，供人工识别数据新鲜度。
...
```text
dynamic_qty
anchor_qty / anchor_date
anchor 后 purchase_in / sales_out / maintenance_out
warehouse snapshots
safety_stock_total / safety_stock_coverage
manual_override_present
```

- 指定目标仓时，只汇总该仓安全库存；未指定时汇总所有仓，并报告 safety stock 已填写行数与
  总行数。部分缺失不得伪装成完整值。
- 现有采购事实没有仓库字段，因此动态库存是型号级事实；分仓快照不能冒充完整分仓流水。
...
`policy_version`、规则实现版本和 HMAC fingerprint，保证历史结论可回放。
...
不得发送客户、供应商、项目名称、合同号、设备号、故障号、原始 justification 或逐单数据。
...
这些是 Agent 控制面动作，不是采购批准或驳回。等待人工最多 7 天；等待时间不计入 active
execution budget，过期按 #226 进入失败终态。获得补充资料或更正 PN 后，创建带
...
1. 采购/销售次数、库存、安全库存、在途状态、active 池和维保事实；
2. 确定性规则结果；
3. Shadow mode 与数据质量限制；
4. AI 解释；
5. Human Interrupt 操作。

不提供“批准”“驳回”“写回采购单”按钮，并固定显示“系统建议不代表业务审批”。

## 16. 审计

记录 Task/Plan/Step/Event/Interrupt ID、actor、workflow/rule/policy 版本、Evidence fingerprint、
