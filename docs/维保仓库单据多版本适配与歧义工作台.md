...
完整、有序的 `(内部编码, 业务表头)` 合同用于判断是否为已经审核过的模板版本，SHA-256 用于展示、token 绑定和审计。必填集合仍匹配、但完整合同没有经过代码审查批准或发生任何增删、移动、改名时：
...
附件是否受控只按稳定内部字段编码和列位置判断，不依赖中文/英文业务表头；附件值只固化“存在”标记，不保存文件名、路径或可识别内容。Mac 1904 日期系统按工作簿 epoch 转换，不能按 Windows 1900 epoch 偏移。
...
- apply 先按排序后的 `document_type:source_document_id` 集合获取 PostgreSQL transaction advisory lock；因此同一业务单据即使来自不同文件也会串行化。没有可固化单据 ID 时才退回 adapter + 文件 hash 锁。
...
- 现场问题：已确认/已更正的 `maintenance_site_issue_line`，必须同时精确命中 `source_order_id + source_line_id + serial_number + project_id`；
- 坏件返还：`maintenance_bad_return` 的显式 `warehouse_reference` 或 `inbound_reference`，且项目必须一致。

依赖表或所需字段尚未部署、项目关系缺失、SN 缺失或稳定键不能唯一命中时，产生 `integration_blocker`/待人工项，绝不按项目名、日期邻近、PN 或文本描述猜测。非全量角色在项目经理分配契约不可用时查询条件恒为 false；文件级、无项目归属的歧义也不会暴露给项目经理。

## 7. 人工裁决

人工裁决使用 `version` 乐观锁。建立关系时，`ambiguity_type`、`field_code`、`link_kind` 和目标类型必须严格匹配，而且所选 `(target_type, target_id)` 必须来自歧义创建时固化的候选集合并在裁决时仍然存在。每次成功裁决会：
