...
def _enforce_bad_return_access(
    db: Session,
    *,
    return_id: str,
    ctx: UserContext,
    ...
    project_id = db.scalar(
        select(MaintenanceBadReturn.project_id).where(
            MaintenanceBadReturn.return_id == return_id
        )
    )
...
def _enforce_return_obligation_access(
    db: Session,
    *,
    obligation_id: str,
    ctx: UserContext,
    ...
    project_id = db.scalar(
        select(MaintenanceReturnObligation.project_id).where(
            MaintenanceReturnObligation.obligation_id == obligation_id
        )
    )
    if project_id is not None:
        enforce_maintenance_project_access(db, project_id=project_id, ctx=ctx)
...
def _transition(
    *,
    ...
    _enforce_bad_return_access(db, return_id=return_id, ctx=ctx)
    operator = _real_operator(db, ident)
    try:
...
def resolve_return_category(
    ...
    _enforce_return_obligation_access(
        db,
