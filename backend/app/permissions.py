...
PERMISSION_META: dict[str, dict] = {
    ...
    "action_maintenance_wbdd_import": {
        ...
        "sensitivity": "critical",
        "risk": "写维保需求单事实、项目身份与来源单挂靠并触发全表成本重算；默认关闭，仅名单勾选。",
    },
    "action_maintenance_expense_collection_upload": {
        "label": "报销/回款工作簿上传覆盖",
    ...
    "page_replenishment_beta": {
        "label": "补库申请",
        "summary": "可打开补库申请页面，在统一维保项目范围内准备前置库补库申请。",
        "can": "打开补库申请页面；同时具备价格数据权限时，可查看当前项目范围内的本人云端草稿，以及按 owner 规则可见的申请与历史版本。",
        "cannot": "搜索价格事实还需「池价格治理」，维护/提交还需「补库申请创建与复提」；不代表可回写审核结果。",
        "typical": ["显式授权的管理员、老板、销售或维保负责人"],
        "sensitivity": "high",
        "risk": "页面、价格数据、申请操作是三层门禁；非 FULL_SCOPE 角色不会仅因职位扩大项目范围，admin/boss 保留平台既有全项目范围。admin 仍需实名页面白名单，但保留系统级动作与数据权限兼容语义。",
    },
    "page_maintenance_beta": {
        "label": "维保管理",
    ...
    "action_replenishment_create": {
        "label": "补库申请创建与复提（按维保项目范围）",
        "summary": "允许在当前统一维保项目可见范围内维护本人补库购物车、提交并复提。",
        "can": "在当前项目范围内新增/更新本人云端草稿、提交不可变版本及处理打回；可导出 owner 规则允许查看的冻结复核包。",
        "cannot": "不能跨当前项目范围读取、更新、删除或提交云端草稿，也不能跨范围复提；失去范围后旧草稿保留但统一不可访问。非 admin 不能查看他人申请；不能人工审批，也不会修改库存或自动生成采购/维保事实。",
        "typical": ["显式授权的管理员、老板、销售或维保负责人"],
        "sensitivity": "high",
        "risk": "提交内容会成为留存业务版本；非 admin 必须同时具备补库页面、池价格和本动作权限，每次读写都按当前认证账号的维保项目范围重新校验，提交与复提必须使用稳定幂等键。",
    },
    "action_replenishment_review": {
        "label": "补库审核结果回写",
