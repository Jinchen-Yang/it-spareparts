...
from app.security import UserContext, apply_field_visibility
        ...
        for order, line in db.execute(statement):
            if order.id != previous_order_id:
                orders.append(_formatted_row(orders, (
                    ...
                    order.end_customer, order.demand_type, order.business_type, order.salesperson,
            ...
            if line is not None:
                ...
                lines.append(_formatted_row(lines, (
                    ...
                    "、".join(line.anomaly_flags or []),
                ), {
                    5: "yyyy-mm-dd", 10: "0.00", 11: "0.00",
                    13: "#,##0.00", 14: "#,##0.00",
                }))
    except ExcelCellTooLong:
        orders.close()
        lines.close()
        raise
