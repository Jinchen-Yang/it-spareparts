...
def _v2_parse_site(
    ...
    for row_no, row in enumerate(ws.iter_rows(min_row=2, values_only=True), 2):
        ...
        if is_create:
            ...
            if merged_row is None:
                continue
            issue_no = str(_cell(merged_row, index, "领用单号") or "").strip() or None
            issue_date = _v2_date(_cell(merged_row, index, "领用日期"), row_no=row_no, label="领用", epoch=ws.parent.epoch)
            pn = str(_cell(merged_row, index, "PN") or "").strip() or None
            serial_number = str(_cell(merged_row, index, "SN") or "").strip() or None
            quantity = _v2_decimal(_cell(merged_row, index, "领用数量"), row_no=row_no, label="领用数量")
            flag = str(_cell(merged_row, index, "是否应返还") or "").strip()
            row = merged_row
