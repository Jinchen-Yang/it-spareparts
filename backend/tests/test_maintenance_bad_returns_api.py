...
def test_manager_row_scope_covers_bad_return_search_and_entity_commands(db):
    admin = _client(db, username="bad_return_manager_scope_admin")
    _confirmed, obligation = _one_required_obligation(
        db,
        admin,
        project_id="project-bad-return-manager-other",
        suffix="manager-scope",
    )
    created = admin.post(
        "/api/maintenance/bad-returns",
        json={
            "project_id": obligation.project_id,
            "idempotency_key": "synthetic-return-manager-scope-draft",
            "lines": [{"obligation_id": obligation.obligation_id, "quantity": "1"}],
            "reason": "建立他人项目返还草稿",
        },
    )
    assert created.status_code == 201, created.text
    return_row = created.json()
    own = _project(db, project_id="project-bad-return-manager-own")
    manager = SysUser(
        username="bad_return_scoped_manager",
        role="purchaser",
        display_name="合成坏件返还项目经理",
        password_hash=hash_password("synthetic-password-123"),
        permissions={
            "page_maintenance": True,
    ...
    db.add(
        MaintenanceProjectUserAssignment(
            assignment_id="bad-return-scope-assignment",
            project_id=own.project_id,
            responsibility_type="primary_manager",
            user_id=manager.id,
            assigned_by="bad_return_manager_scope_admin",
    ...
    obligation_search = client.post(
        "/api/maintenance/return-obligations/search",
        json={"project_id": obligation.project_id, "page": 1, "page_size": 20},
    )
    return_rate = client.get(
        f"/api/maintenance/projects/stable/{obligation.project_id}/return-rate"
    )
    ...
    create = client.post(
        ...
        json={
            "project_id": obligation.project_id,
            "idempotency_key": "synthetic-return-manager-scope-create",
            "lines": [{"obligation_id": obligation.obligation_id, "quantity": "1"}],
            "reason": "越权创建必须失败",
        },
    )
    base_command = {
        "project_id": obligation.project_id,
        "version": return_row["version"],
        "reason": "越权状态命令必须失败",
    }
    submit = client.post(
        f"/api/maintenance/bad-returns/{return_row['return_id']}/submit",
        json={**base_command, "idempotency_key": "scope-return-submit"},
    )
    in_transit = client.post(
        f"/api/maintenance/bad-returns/{return_row['return_id']}/in-transit",
        json={
            **base_command,
            "idempotency_key": "scope-return-transit",
            "logistics_reference": "不得写入",
        },
    )
    warehouse_confirm = client.post(
        f"/api/maintenance/bad-returns/{return_row['return_id']}/warehouse-confirm",
        json={
            **base_command,
            "idempotency_key": "scope-return-confirm",
            "warehouse_reference": "不得写入",
        },
    )
    void = client.post(
        f"/api/maintenance/bad-returns/{return_row['return_id']}/void",
        json={**base_command, "idempotency_key": "scope-return-void"},
    )

    assert [
        obligation_search.status_code,
        return_rate.status_code,
        return_search.status_code,
        create.status_code,
        submit.status_code,
        in_transit.status_code,
        warehouse_confirm.status_code,
        void.status_code,
    ] == [403] * 8
    db.expire_all()
    assert db.get(MaintenanceBadReturn, return_row["return_id"]).status == "draft"


def test_return_rate_official_numerator_is_rkd_inbound(db):
    """Q8：官方返还率 = 入库单坏件件数 / (领用 − 不返还)。"""
    from decimal import Decimal

    client = _client(db, username="rkd_official_rate_admin")
    _one_required_obligation(
        db,
        client,
    ...
    # 等价于 RKD 入库单导入 apply 后的坏件返还事实（3 件）
    batch = MaintenanceDocImportBatch(
        batch_id="rkd-rate-batch-1",
        doc_type="rkd_inbound",
        file_hash="h-rkd-rate",
        filename="入库单.xlsx",
    ...
    db.add(
        MaintenanceRkdReturnLine(
            rkd_line_id="rkd-rate-line-1",
            batch_id="rkd-rate-batch-1",
            head_row_id="rkd-rate-head-1",
            project_id="project-rkd-official-rate",
            head_no="RKD-20260810-0001",
            source_ref="rkd:rate-ref-1",
            pn="PN-rate-1",
            qty=Decimal("3"),
            test_result="坏品",
        )
    ...
    rate = client.get(
        "/api/maintenance/projects/stable/project-rkd-official-rate/return-rate"
    ).json()
