...
def test_project_manager_directory_is_owned_assigned_only_and_writes_need_full_scope(db):
    owned_source = _source_order(
        db,
        raw_order_id="WBDD-SYNTH-SCOPED-OWNED",
        order_no="WBDD-SCOPED-OWNED",
        project_raw="本人项目来源单",
    )
    other_source = _source_order(
        db,
        raw_order_id="WBDD-SYNTH-SCOPED-OTHER",
        order_no="WBDD-SCOPED-OTHER",
        project_raw="其他项目来源单",
    )
    unassigned_source = _source_order(
        db,
        raw_order_id="WBDD-SYNTH-SCOPED-UNASSIGNED",
        order_no="WBDD-SCOPED-UNASSIGNED",
        project_raw="未归属来源单",
    )
    owned_project = _project(
        db,
        project_id="00000000-0000-4000-8000-000000000224",
        project_code="MAINT-SYNTH-201-X",
        display_name="本人负责项目",
    )
    other_project = _project(
        db,
        project_id="00000000-0000-4000-8000-000000000225",
        project_code="MAINT-SYNTH-201-Y",
        display_name="其他负责人项目",
    )
    admin = _admin_client(db, "source_assignment_scope_admin")
    owned_assignment = admin.post(
        "/api/maintenance/project-assignments/orders/assign",
        json={
            "project_id": owned_project.project_id,
            "items": [{"source_order_id": owned_source.raw_order_id}],
            "reason": "建立本人项目来源归属",
        },
    ).json()["assignments"][0]
    assert admin.post(
        ...
        json={
            ...
            "items": [{"source_order_id": other_source.raw_order_id}],
            "reason": "建立其他项目来源归属",
        },
    ).status_code == 200

    manager = _permission_client(
        db,
        username="source_assignment_scoped_manager",
        role="purchaser",
        permissions={
            "page_maintenance": True,
            "page_maintenance_beta": True,
    ...
    manager_user = db.scalar(
        select(SysUser).where(SysUser.username == "source_assignment_scoped_manager")
    )
    db.add(
        MaintenanceProjectUserAssignment(
            assignment_id="00000000-0000-4000-8000-000000000205",
            project_id=owned_project.project_id,
            responsibility_type="primary_manager",
            user_id=manager_user.id,
            assigned_by="source_assignment_scope_admin",
            assignment_reason="合成行级范围验证",
        )
    )
    db.commit()
    ...
    visible = manager.get(
        "/api/maintenance/project-assignments/orders",
        params={"assignment_status": "all", "page_size": 20},
    )
    assert visible.status_code == 200, visible.text
    assert [row["raw_order_id"] for row in visible.json()["rows"]] == [
        owned_source.raw_order_id
    ]
    hidden_unassigned = manager.get(
        "/api/maintenance/project-assignments/orders",
        params={"assignment_status": "unassigned"},
    )
    assert hidden_unassigned.status_code == 200
    assert hidden_unassigned.json()["rows"] == []
    assert hidden_unassigned.json()["total"] == 0
    hidden_other = manager.get(
        "/api/maintenance/project-assignments/orders",
        params=[
            ("source_order_id", other_source.raw_order_id),
            ("assignment_status", "all"),
        ],
    )
    assert hidden_other.status_code == 200
    assert hidden_other.json()["rows"] == []

    assign_attempt = manager.post(
        "/api/maintenance/project-assignments/orders/assign",
        json={
            "project_id": owned_project.project_id,
            "items": [{"source_order_id": unassigned_source.raw_order_id}],
            "reason": "非全量范围账号不得新增归属",
        },
    )
    unassign_attempt = manager.post(
        "/api/maintenance/project-assignments/orders/unassign",
        json={
            "items": [{
                "assignment_id": owned_assignment["assignment_id"],
                "expected_version": owned_assignment["version"],
            }],
            "reason": "非全量范围账号不得撤销归属",
        },
    )
...
def test_delete_snapshot_binds_assignment_and_tombstone_removes_every_operation_entry(db):
    source = _source_order(
        db,
        raw_order_id="WBDD-SYNTH-DELETE-ASSIGNMENT",
        order_no="WBDD-DELETE-ASSIGNMENT",
        project_raw="待删除已归属来源单",
    )
    project = _project(
        db,
        project_id="00000000-0000-4000-8000-000000000226",
        project_code="MAINT-SYNTH-201-Z",
        display_name="删除归属联动项目",
    )
    username = "source_assignment_delete_admin"
    client = _admin_client(db, username)
    ...
    before_workspace = client.get(
        f"/api/maintenance/projects/stable/{project.project_id}/workspace",
        params={"as_of": "2026-08-09"},
    )
    assert before_workspace.status_code == 200, before_workspace.text
    assert before_workspace.json()["project"]["manual_source_order_count"] == 1

    now = datetime(2026, 8, 9, 8, 0, tzinfo=timezone.utc)
    intent = maintenance_demands.create_delete_intent(
        db,
        source_order_ids=[source.raw_order_id],
        reason="确认该来源单为误导入整单",
        idempotency_key="delete-assigned-source-order",
        operated_by=username,
        now=now,
    )
    assert intent["items"][0]["active_project_assignment"] == {
        "assignment_id": assignment["assignment_id"],
        "project_id": project.project_id,
    ...
    assert directory.status_code == 200
    assert directory.json()["rows"] == []
    after_workspace = client.get(
        f"/api/maintenance/projects/stable/{project.project_id}/workspace",
        params={"as_of": "2026-08-09"},
    )
    assert after_workspace.status_code == 200, after_workspace.text
    assert after_workspace.json()["project"]["manual_source_order_count"] == 0
    assert client.post(
        "/api/maintenance/project-assignments/orders/assign",
        json={
            "project_id": project.project_id,
...
def test_assignment_change_after_delete_review_makes_whole_delete_batch_stale(db):
    source = _source_order(
        db,
        raw_order_id="WBDD-SYNTH-DELETE-STALE-ASSIGNMENT",
        order_no="WBDD-DELETE-STALE-ASSIGNMENT",
        project_raw="归属变化删除冲突来源单",
    )
    first_project = _project(
        db,
        project_id="00000000-0000-4000-8000-000000000227",
        project_code="MAINT-SYNTH-201-AA",
        display_name="删除复核时项目",
    )
    next_project = _project(
        db,
        project_id="00000000-0000-4000-8000-000000000228",
        project_code="MAINT-SYNTH-201-AB",
        display_name="复核后改派项目",
    )
    username = "source_assignment_delete_stale_admin"
    client = _admin_client(db, username)
    first = client.post(
        "/api/maintenance/project-assignments/orders/assign",
        json={
    ...
    intent = maintenance_demands.create_delete_intent(
        db,
        source_order_ids=[source.raw_order_id],
        reason="建立绑定归属版本的删除复核",
        idempotency_key="delete-stale-assignment",
        operated_by=username,
        now=now,
    )
    db.commit()
