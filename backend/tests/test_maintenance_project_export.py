...
def export_client(db, *, username: str, with_profit: bool = True):
    return client_for(db, username=username, role="readonly", overrides={
        "page_maintenance": True,
        "page_maintenance_boss": True,
...
def test_export_options_return_whitelist_and_exact_default_fields(db):
    response = export_client(db, username="project-export-options").get(
...
def test_export_options_and_post_filter_profit_fields_by_server_permission(db):
    ...
    filtered = client.post(
        "/api/maintenance/boss-board/projects/export",
        json={"fields": ["project_name"], "sort": "cost_ratio"},
    )
    assert filtered.status_code == 422
    assert filtered.json()["detail"]["code"] == "cost_contract_permission_required"


def test_export_rejects_unknown_field_instead_of_resolving_orm_attribute(db):
    response = export_client(db, username="project-export-unknown").post(
        "/api/maintenance/boss-board/projects/export",
...
def test_export_xlsx_uses_current_contract_and_latest_confirmed_collection(db):
    ...
    response = export_client(db, username="project-export-xlsx").post(
        "/api/maintenance/boss-board/projects/export",
