"""近期加权成交价参考（销售出价用）：近 N 条且 D 天内成交价，按名次线性加权。

需求（用户 2026-06-12 确认）：不给"建议售价"（销售自行把握加价），只给一个稳的
成交价参考——近5条且30天内，最近一条权重最高、依次递减（越近越高），削单笔异常价方差。

日期全部用 today-相对（确定可复现，不随运行日漂移）。
"""
...
def test_ref_price_window_counts_revenue_and_exact_weighting(db):
    ...
    # 倒序 100/110/130，名次线性权重 3/2/1：ref = (3*100+2*110+1*130)/6 = 650/6 = 108.33
    # 朴素均值=113.33，近端权重高 → 参考价偏向最近的 100
    assert abs(r["ref_sale_price"] - 108.33) <= 0.01, f"加权值不符: {r['ref_sale_price']}"
