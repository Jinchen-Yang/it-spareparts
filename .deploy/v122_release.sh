...
readonly EXPECTED_FROM=d9f1a3c7e5b2
readonly EXPECTED_TO=f9b2d4e7c1a6
readonly DEFAULT_APP_DIR=/home/ubuntu/apps/it-spareparts
readonly LOCK_DIR=/run/lock/it-spareparts-v122-beta
...
readonly SCRIPT_DIR
readonly MANIFEST_TOOL="$SCRIPT_DIR/v122_beta_manifest.py"
...
usage() {
  cat >&2 <<'EOF'
usage: v122_beta_release.sh PACKAGE_DIR EVIDENCE_DIR COMMAND [ARGS]
...
[ "$(stat -c '%a %U:%G %h' "$SCRIPT_DIR/v122_beta_release.sh")" = "700 root:root 1" ] \
...
for key in replacements:
    if key not in seen:
        output.append(f"{key}={replacements[key]}")
fd, temporary = tempfile.mkstemp(prefix=".env-v122-", dir=path.parent)
try:
...
while IFS= read -r -d '' package_file; do
  ...
  case "$(basename -- "$package_file")" in
    v122_beta_manifest.py|v122_beta_release.sh) package_mode=700 ;;
  esac
...
fd, temporary = tempfile.mkstemp(prefix=".v122-", dir=destination.parent)
try:
    with os.fdopen(fd, "w", encoding="utf-8") as stream:
...
state.update({
    "format": "v122-beta-release-state-1",
...
install_candidate_compose() {
  ...
  install -m 600 -- "$ACTIVE_COMPOSE" "$EVIDENCE_DIR/pre-v122-compose.yml"
  temporary=$(mktemp -- "$APP_DIR/.docker-compose.v122.XXXXXX")
...
tag_candidate_images() {
  ...
  docker image tag "$OLD_APP_IMAGE_ID" "it-spareparts-release/app:pre-v122-$MANIFEST_SHA"
  docker image tag "$OLD_FRONTEND_IMAGE_ID" "it-spareparts-release/frontend:pre-v122-$MANIFEST_SHA"
  docker image tag "$NEW_APP_IMAGE_ID" "$APP_IMAGE_REF"
  docker image tag "$NEW_FRONTEND_IMAGE_ID" "$FRONTEND_IMAGE_REF"
  [ "$(docker image inspect -f '{{.Id}}' "$APP_IMAGE_REF")" = "$NEW_APP_IMAGE_ID" ]
  [ "$(docker image inspect -f '{{.Id}}' "$FRONTEND_IMAGE_REF")" = "$NEW_FRONTEND_IMAGE_ID" ]
...
print(json.dumps({"format":"v122-preflight-1","manifest_sha256":sys.argv[1],
...
backup_restore() {
  ...
  restore_name="it-spareparts-v122-restore-${MANIFEST_SHA:0:12}"
...
import datetime as dt,json,sys
print(json.dumps({"format":"v122-backup-restore-1","manifest_sha256":sys.argv[1],
...
import datetime as dt,json,sys
print(json.dumps({"format":"v122-migration-1","manifest_sha256":sys.argv[1],
...
if (row.get("format"),row.get("manifest_sha256"),row.get("minute"),row.get("result")) != (
    "v122-observation-1",sys.argv[2],int(sys.argv[3]),"passed"
):
...
import datetime as dt,json,sys
print(json.dumps({"format":"v122-observation-1","manifest_sha256":sys.argv[1],
...
rollback_app() {
  ...
  state_update old_images_on_d9 \
    '{"beta_contained":true,"database_restore_forbidden":true,"database_revision":"f9b2d4e7c1a6"}'
