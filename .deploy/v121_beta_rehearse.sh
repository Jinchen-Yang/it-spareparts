...
[ "$#" -eq 10 ] || fatal \
  "usage: v121_beta_rehearse.sh DUMP TARGET_SHA PARENT_PROD_SHA DB_IMAGE_ID NEW_APP_IMAGE_ID OLD_APP_IMAGE_ID OLD_FRONTEND_IMAGE_ID STABLE_CREDENTIAL_JSON forward-only|test-old-images OUTPUT_DIR"
DUMP=$(realpath -e -- "$1")
TARGET_SHA=$2
PARENT_SHA=$3
DB_IMAGE_ID=$4
...
CREDENTIAL=$(realpath -e -- "$8")
MODE=$9
OUTPUT_DIR=$(realpath -m -- "${10}")
readonly DUMP TARGET_SHA PARENT_SHA DB_IMAGE_ID NEW_APP_IMAGE_ID
readonly OLD_APP_IMAGE_ID OLD_FRONTEND_IMAGE_ID CREDENTIAL MODE OUTPUT_DIR
...
readonly WORK DB_NAME APP_NAME NETWORK_NAME
cleanup() {
  status=$?
...
python3 - "$WORK/output/production-copy-migration-rehearsal.json" \
  ...
  "$(sha256sum "$PRESSURE" | awk '{print $1}')" "$DB_IMAGE_ID" <<'PY'
...
payload={"format":"v121-production-copy-migration-rehearsal-v1",
 ...
 "database_image_id":sys.argv[10],"statement_timeout_ms":120000,"lock_timeout_ms":5000,
...
if [ "$MODE" = test-old-images ]; then
  docker run -d --name "$APP_NAME" --network "$NETWORK_NAME" --network-alias app \
    -p 127.0.0.1::8000 \
    -e DATABASE_URL=postgresql+psycopg://spareparts@db:5432/spareparts \
    -e ENVIRONMENT=dev -e ADMIN_PASSWORD=rehearsal-only-not-production \
    -e SECRET_KEY=rehearsal-only-not-production-secret \
    "$OLD_APP_IMAGE_ID" >/dev/null
  APP_PORT=$(docker port "$APP_NAME" 8000/tcp | sed -n 's/^127\.0\.0\.1:\([0-9][0-9]*\)$/\1/p')
  [[ "$APP_PORT" =~ ^[0-9]+$ ]] || fatal "cannot resolve isolated old-app loopback port"
  for attempt in $(seq 1 60); do
    if python3 - "$CREDENTIAL" "http://127.0.0.1:$APP_PORT" >/dev/null 2>&1 <<'PY'
...
def request(path,payload=None,token=None):
    ...
    with urllib.request.urlopen(urllib.request.Request(origin+path,headers=headers,data=data),timeout=5) as r:
...
PY
  ...
  python3 - "$WORK/output/old-images-on-d9-rehearsal.json" "$TARGET_SHA" \
    "$PARENT_SHA" "$OLD_APP_IMAGE_ID" "$OLD_FRONTEND_IMAGE_ID" <<'PY'
import datetime as dt,json,os,sys
